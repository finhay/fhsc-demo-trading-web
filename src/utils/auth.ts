import { AUTH_REGEX, SSO_ALLOWED_REDIRECT_URIS } from '@/constants/auth';
import type { ValidationType } from '@/types/auth/otp';
import type { AuthValidationMessages } from '@/types/pages/auth';

export const isValidSsoRedirectUri = (uri: string): boolean => {
    try {
        const url = new URL(uri);
        return SSO_ALLOWED_REDIRECT_URIS.includes(`${url.origin}${url.pathname}`);
    } catch {
        return false;
    }
};
