...
export const createChartAssetSparkline = (
    ...
    return {
        ...
        tooltip: {
            ...
            axisPointer: {
                type: 'line',
                lineStyle: { color: CHART_COLORS.series.primary, width: 1, type: 'dashed' },
            },
            formatter: (params) => {
                const param = (
                    Array.isArray(params) ? params[0] : params
                ) as echarts.DefaultLabelFormatterCallbackParams & { axisValue?: string };
                const value = Number(param.value);
                const date = String(param.axisValue ?? '');
                return `${formatDate(date)}<br/><b>${formatNumberVN(value, {
        ...
        series: [
            {
                ...
                data: values,
                lineStyle: { width: 2, color: CHART_COLORS.series.primary },
                areaStyle: {
                    color: {
                        type: 'linear',
                        x: 0,
                        ...
                        y2: 1,
                        colorStops: [
                            { offset: 0, color: CHART_COLORS.series.gradientStart },
                            { offset: 1, color: CHART_COLORS.series.gradientEnd },
                        ],
                    },
                },
            },
