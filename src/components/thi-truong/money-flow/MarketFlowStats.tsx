...
export const MarketFlowStats = ({ stats, showDot = false, isLoading = false }: Props) => {
    ...
    const renderValue = (value: string, colorClass: string) =>
        isLoading ? (
            ...
            <span className={`body-4-highlight ${colorClass}`}>{value}</span>
        );
                ...
                <>
                    <div className="flex w-full flex-col gap-3">
                        <div className="flex items-center gap-1">
                            <div className="flex items-center gap-2">
                                {showDot && <MarketDot />}
                                <span className="body-4-highlight text-primary">{'GTGD'}</span>
                            </div>
                            <span className="body-4 text-tertiary">({'tỷ đồng'})</span>
                        </div>
                        <div className="flex w-full items-center gap-6">
                            ...
                            <div className="flex min-w-0 flex-1 flex-col gap-1">
                                <span className="body-4 text-secondary">{'Ròng'}</span>
                                {renderValue(
                    ...
                    <div className="flex w-full flex-col gap-3">
                        <div className="flex items-center gap-1">
                            <div className="flex items-center gap-2">
                                {showDot && <MarketDot />}
                                <span className="body-4-highlight text-primary">{'KLGD'}</span>
                            </div>
                            <span className="body-4 text-tertiary">({'cp'})</span>
                        </div>
                        <div className="flex w-full items-center gap-6">
                            <div className="flex min-w-0 flex-1 flex-col gap-1">
                                <span className="body-4 text-secondary">{'Mua'}</span>
