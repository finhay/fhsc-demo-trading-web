...
import { fetchHayBondPackagesDetail } from '@/services/api/bond-enterprise/packages';
...
export const HaybondPackageDetail = () => {
    const trans = useTranslate();
    const router = useRouter();
    const { id } = router.query;
    const [data, setData] = useState<PackageDetailData | null>(null);
    const [isLoading, setIsLoading] = useState(true);
    const { isOverlayLoading, setTermBuyData, openBuy } = useHaybondStore();

    const handleOpenOrderBuy = (packageId: number, name: string, interestRate: number) => {
        setTermBuyData({
            packageInfo: { id: packageId, name, interest_rate: interestRate },
        });
        openBuy(packageId);
    };
    ...
    const fetchDetail = async () => {
        ...
        setIsLoading(true);
        try {
            const { error_code, message, data: detail } = await fetchHayBondPackagesDetail(id);
