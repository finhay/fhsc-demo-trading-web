    ...
    stages {
        stage('Setup') {
            steps {
                script {
                    env.K8SCLS = 'cls2'
                    env.WORKSPACE = 'products'
                    env.DOCKERFILE = 'Dockerfile'
                    env.DEPLOYMENT = 'products-demo-trading-web-alpha'
                    env.IMAGES = 'demo-trading-web-alpha'
                ...
                script {
                    try {
                        if ("$WORKSPACE"=="products") {
                            sh '''
                                sudo cp ../demo-stg-update-env/demo-trading-web/.env.alpha ./.env
                                ls -lah
                            '''
                        }
                    }
        ...
        stage('Build and upload image') {
            steps {
                script {
                    try {
                        sh 'bash ./cicd/build_image.sh $WORKSPACE ${DEPLOYMENT} ${IMAGES}  $BUILD_NUMBER ${URL_PRIVATE_REGISTRY}'
        ...
        stage('Promote to Deployment') {
            steps {
                script {
                    try {
                        sh 'bash ./cicd/update_image_to_service_on_k8s.sh $WORKSPACE ${DEPLOYMENT} ${IMAGES}  $BUILD_NUMBER ${URL_PRIVATE_REGISTRY} $K8SCLS'
                    }
                    catch (err) {
                        currentBuild.result = "FAILED"
                        throw err
